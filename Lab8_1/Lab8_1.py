...
# ПОСТРОЕНИЕ СЛУЧАЙНОГО ЛЕСА(решает проблему переобучения - усреднение)
...
# М.ВЫЧИСЛ.ВАЖНОСТИ ПРИЗНАКОВ (агрегир.знач.важности по всем деревьям)
# более шир.картинка
...
# ГРАДИЕНТНЫЙ БУСТИНГ ДЕРЕВЬЕВ РЕГРЕССИИ (рак)
...
# ВИЗУАЛИЗ.ВАЖНОСТИ ПРИЗНАКОВ (более глуб.представл)
gbrt = GradientBoostingClassifier(random_state=0, max_depth=1)
gbrt.fit(X_train, y_train)
def plot_feature_importances_cancer(model):
    n_features = cancer.data.shape[1]
...
# похож.на важности получ.с пом.случ.леса (но проигнорили нек.признаки)
plt.show()
